...
A macOS menu bar app that shows every Claude Code session's prompt-cache countdown, cost, and cache-hit
ratio in real time, and can focus the matching Ghostty tab. Built against `spec.md` in this repo — read
that first for the full design rationale; this file covers build/run and what changed on contact with a
real machine.

## Build & run

This ships as a **Swift Package** (not a hand-written `.xcodeproj`) with an executable target. Two ways
to build it:

### Option A — Xcode (recommended, gives you a real `.app`)
...
Then build with `xcodebuild` from the command line:

```bash
xcodebuild -scheme ClaudeSessionMonitor -destination 'platform=macOS' build
```

**Note:** this repo was built in an environment with only Xcode Command Line Tools installed, not full
Xcode (`xcodebuild -version` reports "requires Xcode"), so the `xcodebuild` invocation above could not be
executed here. It follows Apple's documented Swift-package-as-Xcode-project support and should work
as-is once you have full Xcode — if the scheme name differs, `xcodebuild -list` from this directory will
show the generated scheme name to use instead.

### Option B — command line only, no Xcode

```bash
swift build -c release        # verified in this environment: compiles cleanly
./Scripts/build_app.sh        # wraps the binary into ClaudeSessionMonitor.app (see below)
open .build/ClaudeSessionMonitor.app
```

`Scripts/build_app.sh` exists because a bare SPM executable has no `CFBundleIdentifier`. That's not just
cosmetic: **`UNUserNotificationCenter` crashes the process outright** (`bundleProxyForCurrentProcess is
nil`) if you run the raw binary directly — this was caught by actually running the built binary during
development. `NotificationService` now guards against that case so `swift run` won't crash, but real
notifications require the proper `.app` bundle the script produces. This was run end-to-end here: the
packaged `.app` launched, ran a full transcript-scan + `ccusage` refresh cycle, and quit cleanly.

Gatekeeper note: this build is unsigned. First launch of `.build/ClaudeSessionMonitor.app` via double-click
may be blocked; right-click → Open, or run `xattr -dr com.apple.quarantine .build/ClaudeSessionMonitor.app`
first. No Apple Developer account is required or used anywhere in this project.

## Permissions

- **Automation (Ghostty control):** first time the app tries to focus a tab, macOS prompts for Automation
  permission. If denied or not yet granted, the focus button/row-tap is simply disabled — grant it later
  via System Settings → Privacy & Security → Automation → Claude Session Monitor → Ghostty.
...
- Scans `~/.claude` for transcript files, computes each session's prompt-cache countdown against a TTL
  (5 or 60 min, your setting, or an exact per-session value when detected — see below), and shows the
  soonest-to-expire session's countdown in the menu bar.
- Dropdown lists every session from the last 24h (no scroll — see "Deviations"), sorted so warm/expiring
  sessions always sort above cold ones: status dot, project name, Claude Code's own AI-generated title
  for the session (the same label it uses as the Ghostty tab title), countdown, `ccusage` cost, cache-hit
  ratio. Hover a row for the full detail: working directory, TTL source, last-turn time, raw token counts,
  whether a matching Ghostty tab was actually found, whether a `claude` process is actually still running
  in that directory, and which built-in tools / MCP plugins / Skills that session used. **Click a row** to
  focus that tab — only shown live (not just "Ghostty is running somewhere") when a real matching tab
  exists. **Right-click** for "Paste /handoff" / "Paste /compact" — pastes into the terminal (never runs
  it for you).
- **Top-of-screen banner** + a system notification when a warm session is about to go cold, with "Switch
  to Session" and "Handoff" buttons.
- Gear icon → **Settings** opens as its own window (via SwiftUI's `Settings` scene + `SettingsLink`, the
  same pattern Docker Desktop's menu bar icon uses for Preferences), not a sheet crammed into the
  dropdown: TTL, refresh interval, expiring-soon threshold, notification lead time, Ghostty toggle — all
  persisted via `UserDefaults`.

## Transcript layout found on this machine
...
row plus a footer note, never a crash. (Aside, unrelated to this app: on this machine `npx` itself failed
until `npm_config_cache` pointed at a writable directory, because `~/.npm` had root-owned cache files —
a pre-existing local `npm` issue, not something this app should paper over.)
...
2. **Detected-TTL tag.** An "auto" tag replaces the optional heuristic in §6 with an exact per-turn signal
   already in the transcript (`cache_creation.ephemeral_{1h,5m}_input_tokens`) — see below.
...
10. **Not verified live: Ghostty AppleScript `focusTab`.** Ghostty *is* installed and running on this
    machine and `hasOpenTab`/`refreshAvailability` were exercised for real (confirmed via the debug scan
    log — no dependency warnings, meaning the automation probe succeeded), but clicking a row to actually
    focus a tab was not clicked through interactively during this build. Treat `focusTab` itself as the
    least-tested path until you've clicked a focus arrow against a real Ghostty window.
...
19. **`rtk gain` integration, plus a "Hooks" usage category.** Asked directly whether the transcript
    ...
    - Hooks (like `rtk`, the user's own Bash-rewriting CLI — see `~/.claude/RTK.md`) are a distinct
      mechanism from MCP/Skills: they run transparently around a tool call rather than being something the
      model chose to invoke, so they never appear as a `tool_use` block — only as their own `attachment`
      event (`{"type": "attachment", "attachment": {"type": "hook_success", "command": "rtk hook claude",
      ...}}`, confirmed by grepping a real transcript). `ToolUsage.hooks` tallies these by the hook
      binary's name (the first word of `command`), shown as its own "Hooks" category right next to
      Plugins/MCP in the detail panel, per direct request.
    - Separately, real per-project stats: `rtk gain -p -f json` (confirmed by running it directly in this
      ...
      command, since `rtk` here is the user's own personal tool with no public install path this app
      could assume for anyone else running it.
...
25. **`xcodebuild` build step not executed.** Full Xcode isn't installed in the build environment (only
    ...
    the `xcodebuild` command above is the documented equivalent per Apple's SPM-as-Xcode-project support
    but wasn't run here — see Option A.

## Debugging

The built app writes one line per scan to stderr: `[ClaudeSessionMonitor] scan: N session(s), warnings:
[...]`. Nothing else surfaces this for a menu-bar-only app with no console, so if the dropdown looks wrong
run it from Terminal to see it directly:

```bash
.build/ClaudeSessionMonitor.app/Contents/MacOS/ClaudeSessionMonitor
```

## Source layout

Matches `spec.md` §7 with the one addition noted above (`Services/SettingsStore.swift`):
